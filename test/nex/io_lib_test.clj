    ...
    (let [tmp-dir (io/file (System/getProperty "java.io.tmpdir") (str "nex-dir-io-" (System/nanoTime)))
      ...
      (try
        (let [output (with-out-str
                       ...
                       (repl/eval-code ctx "let backup: Directory := root.child_dir(\"backup\")")
                       (repl/eval-code ctx "data.copy_to(backup)")
                       (repl/eval-code ctx "print(backup.exists())")
                       (repl/eval-code ctx "let moved: Directory := root.child_dir(\"moved-data\")")
                       (repl/eval-code ctx "backup.move_to(moved)")
                       (repl/eval-code ctx "print(moved.exists())")
                       (repl/eval-code ctx "print(root.exists())")
    ...
    (let [tmp-dir (io/file (System/getProperty "java.io.tmpdir") (str "nex-bin-io-" (System/nanoTime)))
      ...
      (try
        (let [output (with-out-str
                       ...
                       (repl/eval-code ctx "writer.write(\"ABCD\".to_bytes())")
                       (repl/eval-code ctx "print(writer.position())")
                       (repl/eval-code ctx "writer.seek(1)")
                       (repl/eval-code ctx "writer.write(\"Z[\".to_bytes())")
                       (repl/eval-code ctx "print(writer.position())")
                       (repl/eval-code ctx "writer.close()")
