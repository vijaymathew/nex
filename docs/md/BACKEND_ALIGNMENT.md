...
| C3a | Typechecker gap noticed while fixing sample.nex: `Parent.field` (class-qualified parent-field access) passes the typechecker but is rejected by both engines at different stages (interpreter at runtime, compiler at lowering). Should be a typecheck error; `Parent.routine(...)` is the supported form. **Fixed**: `check-class-constant-access` (nex.typechecker) now rejects `Parent.field` at type-check time, naming the field and pointing at the supported `Parent.routine(...)` alternative, instead of falling through to `Any` unchecked. (A field has no dispatch to disambiguate the way a routine does — and multiple inheritance genuinely duplicates a common ancestor's field per branch rather than sharing one slot, so there is no single value `Parent.field` could even name.) | done |
