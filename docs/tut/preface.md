...
Every code example in this book can be typed at the Nex REPL and will produce the output shown. No example requires installation beyond accessing the browser-based IDE at [schemer.in/nex](https://schemer.in/nex). The exercises at the end of each chapter are meant to be done, not read. The ones marked with an asterisk extend the chapter's ideas into territory that is not fully covered — they are invitations to explore rather than tests of recall.
