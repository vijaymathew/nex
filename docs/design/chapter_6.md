...
## 6.1 The Front End and Alternative Hosts

The parser is built around ANTLR and `clj-antlr`, which work well on the JVM but tie the front end to a JVM process. Nex currently targets the JVM only, so this is not a limitation in practice — but any future effort to run Nex on another host (for example, a browser or a native runtime) would first have to answer how source text becomes an AST off the JVM. A pure-Clojure(Script) parser, a generated parser pipeline, or a principled AST-transport boundary are the plausible paths, and none is trivial.

The current boundary is defined in [`src/nex/parser.clj`](https://github.com/vijaymathew/nex/blob/main/src/nex/parser.clj) and [`src/nex/walker.clj`](https://github.com/vijaymathew/nex/blob/main/src/nex/walker.clj). A contributor working on this area should read both before proposing a direction, because the constraints they reveal are easy to underestimate.



## 6.2 Libraries and Package Management
...
## 6.5 Backends and Deployment

Nex separates its front end (parser, walker, typechecker) from its back end (lowering, IR, and the JVM bytecode compiler) cleanly enough that the execution strategy is not baked into the language. The questions any future backend would have to answer — how values are represented, how classes and functions are lowered, how contracts are enforced, how concurrency is expressed, how host integration is handled — are answered once, in a form a second backend could follow.

A WebAssembly-oriented backend or a native-code path is conceivable further out. None of these should be pursued speculatively — the architecture supports them, but that is not by itself a reason to build them.

The pressure points for new backends are concentrated in the lowering pass and the backend entry point: lowering in [`src/nex/lower.clj`](https://github.com/vijaymathew/nex/blob/main/src/nex/lower.clj) and JVM emission in [`src/nex/compiler/jvm/file.clj`](https://github.com/vijaymathew/nex/blob/main/src/nex/compiler/jvm/file.clj). Reading both before designing a new backend is not optional — the decisions they embody are easy to reinvent badly.



## 6.6 Language Growth and Restraint
...
Implementation details matter here in a way that design discussions often ignore. A proposal that is elegant at the language level may require special cases in every pass — grammar, walker, typechecker, interpreter, and the JVM backend — in ways that accumulate into an implementation that is no longer easy to read. Nex should continue to prefer features that fit the existing architecture cleanly, not because cleanliness is an end in itself, but because a messy implementation makes the semantics harder to verify and the codebase harder for contributors to enter.



## 6.7 What Maturity Looks Like

A mature Nex would not necessarily be a larger Nex. It would be a more settled one: clearer library conventions, stronger tooling, more complete documentation, a typechecker and runtime that are broader in coverage but still readable in full.

That last property matters more than it might seem. The codebase is currently small enough that a determined reader can trace any feature from grammar to walker to typechecker to interpreter to the JVM backend. That traceability is one of Nex's most valuable properties. Growth should preserve it — not out of sentiment, but because a language designed to teach disciplined thinking about programs should itself be implemented in a way that rewards disciplined reading.
